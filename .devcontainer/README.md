...
- **Base image**: `python:3.11-slim`, matching `.python-version`.
- **Features**:
  - `common-utils`: git, curl, zsh and oh-my-zsh, with zsh as the default shell. `username` is `none`, so the container keeps running as `root` the way it did before.
  - `uv`: the Python package manager the `Makefile` and CI depend on.
- **devcontainer.json**: the extension recommendations this project already had, plus a zsh terminal profile.
