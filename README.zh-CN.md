...
- **Python 3.10+**（已在 3.10、3.11、3.12 上测试）
...
3. **安装 Playwright 浏览器**（可选，用于浏览器自动化）：
...
```yaml
enable: true
target: com.longe.allstarhmt    # Android 包名
host: 127.0.0.1                # ADB 主机地址
serial: '16416'                 # ADB 端口（组成 127.0.0.1:16416）
image_list:
  - image_name: 开始配对
...
通过 Playwright 实现网页自动化：

```yaml
enable: true
target: https://example.com     # 目标 URL
host: ''                        # 浏览器模式留空
serial: ''                      # 浏览器模式留空
image_list:
...
3. **图像检测**：使用 OpenCV 模板匹配寻找 UI 元素
4. **执行操作**：按照配置的延迟点击检测到的元素
5. **通知推送**：在完成或出错时发送 Discord 更新
...
- 安装 Google Chrome（Playwright 依赖）
- 检查目标 URL 是否可访问
- 确认网络连接正常
- 如需自定义浏览器，在 `cores/screenshot.py` 中修改 `channel` 参数
